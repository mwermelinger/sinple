...
The latest version of this file can be found on http://michel.wermelinger.ws. 

Graph representation
--------------------
A simple digraph is represented by a pair `(nodes, arcs)`, where `nodes` is the set of unique node names (e.g. integers or strings) and `arcs` is the set of arcs. Each arc is a pair of node names `(tail, head)`, stating there is a directed arc from the `tail` node to the `head` node.

In the rest of the code and documentation, we use the word graph instead of digraph.
"""

# Example graphs
# --------------
# The following graphs will be used as unit tests for functions. 

# The **empty graph** has no nodes and hence no arcs.
EMPTY = (set(), set())		# {} is the empty dictionary

# The **trivial graph** has a single node and no arcs.
TRIVIAL = ({1}, set())

# The smallest non-simple graph.
LOOP = ({1}, {(1,1)})

# A **cycle graph** has nodes connected in a 'round-robin' fashion. 
...
# ### Exercises
#
# 1. Define K1 and K3, the complete graphs with one and three nodes, respectively.
# 2. Define C2, the cycle graph with two nodes.
...
def arcs(graph):
	"""Return the set graph's arcs as a set."""
	(nodes, arcs) = graph
	return arcs
	
def nodes(graph):
	"""Return the graph's nodes as a set."""
	(nodes, arcs) = graph
	return nodes
...
def make_arc(tail, head):
	"""Return an arc from tail to head node."""
	return (tail, head)
...
assert(degree(1, graph) == 1)
assert(degree(2, graph) == 2)
assert(degree(1, empty) == 0)
